...
def render_coupon_card(user: User, menu: Menu, plan: Plan) -> None:
    ...
    with st.container(border=True):
        ...
        if not st.session_state.get(f"opened-{plan.id}"):
            ...
            if b1.button("クーポンを使って予約する", key=f"cp-{plan.id}", type="primary", use_container_width=True):
                acquire(user, plan)
                if plan.member_url:
            ...
            if plan.member_url:
                b1.link_button("予約ページを開く", plan.member_url, type="primary", use_container_width=True)
            if b2.button("口コミを書く", key=f"post-{plan.id}", use_container_width=True):
